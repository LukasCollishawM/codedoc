...
The format is 1.0 and does not change incompatibly. A later 1.x may add an optional member omitted at its default, a vocabulary entry, a rung below the existing ones, or a digest domain; it may not change any rule in section 1, any domain string, any fingerprint pre-image, any frozen wire string, the meaning or position of a rung, or whether a member is required. A major version is a new format rather than an amendment, and needs a mechanical `codedoc migrate` path before any record is rewritten. A user's ledger is their institutional memory, accumulated over years and not reproducible — corrupting it is unforgivable in a way that breaking an API never is. When a format change and an ergonomics win are in tension, the format wins.
