...
- **G5** **Dogfood.** This repository contains zero comments, carries its own architecture in its own ledger, and CI runs `codedoc doctor` against that ledger on every push. 48 records over 816 declarations: the point is not the percentage but that every invariant argued for in this file is also recorded where the code is, and that the tool is used the way it asks others to use it. The project is not real until it is its own first user.
