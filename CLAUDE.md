...
- **G2** Zero false reattachments. Measured over **ripgrep**, 400 commits and 1,073 anchors: **98.2% survival, 0 suspicious, and all 19 detachments audited as correct** — `TEMPLATE_CHOICES` and `version_pcre2` were deleted outright, the `impl Default for …` blocks were replaced by derives, and the three `#[cfg]`-gated `is_hidden` definitions were renamed. Every anchor either found its code or correctly reported it gone. Also over **httpx**, 300 commits of Python and 502 anchors: **92.8% survival, 0 suspicious**, with the sampled detachments all confirmed deleted — httpx removed most of `_utils.py` over that range, so the lower figure is a codebase that deleted more, not an adapter that tracks worse. Also over this repository's own history — 45 commits including a crate extraction, a module moved between crates and several renames — **229 anchors, 97.8% survival, 0 suspicious** — and every one of the five detachments names a construct that was genuinely deleted (`command_verify`, the `operations` module, two argument structs replaced in a rewrite), so detachment was correct in all five cases rather than a survival failure. **The hard zero is carried by the property test**, which has ground truth by construction: for any tree and any edit script, resolution is correct or `Detached`. Replay over real history cannot label outcomes automatically, so it measures survival and *flags* confident rungs landing on a different symbol for human inspection. Do not claim replay proves the invariant; it evidences it.
