...
- **`codedoc_render` and `codedoc_import` for agents**, bringing the MCP surface to fourteen tools. Asked for onboarding notes, an agent can now generate them from verified anchors instead of from its own reading of the code. The five remaining CLI-only commands are administrative by intent.
